...
var PIKENG =  function ( parameters ){
	...
	_NewInputPosition                  = new THREE.Vector2(0,0);
	_VROn                              = false;
	_moving                            = false;
	_zooming                           = false;
	_debug                             = false;
	...
	var _Pivoting                      = false;
	var _Zooming                       = false;
	var _InputScale                    = { scale:0, timeStamp:null };
	var _NewInputScale                 = { scale:0, timeStamp:null };
	var _InputTime                     = 0;
	...
	this.Limits                        = { rotation:  { x:{max:null, min:null}, y:{max:null, min:null}, z:{max:null, min:null} }, 
	                                       radius:    { max:null, min:null }, fov:{ max:null, min:null }, 
	                                       pan:       { x:{max:null, min:null}, y:{max:null, min:null}, z:{max:null, min:null} } 
	                                     }; // Future limits implementation.
	this.Speeds                        = { zoomFOV:{ mouse:3, touch:3 }, zoomRadius:{ mouse:3000, touch:3000 }, pan:{ touch:5, mouse:5 }, pivot:{ touch:20, mouse:20 } };
	this.PivotVelocity                 = { dx:0, dy:0 };
	this.PanVelocity                   = { dx:0, dy:0 };
	this.ZoomVelocity                  = 0;
	this.PivotDeceleration             = 5;
	this.PanDeceleration               = 10;
	this.ZoomDeceleration              = 20;
	this.DeviceOrientationEnabled      = false;
	this.DeviceMotionEnabled           = false;
	this.CameraNormalised              = null;
	this.MoveCancelsCamAnim            = true;
	this.TouchBlocker                  = null; // Block touches after touch end, stops random spinning that users don't want
	this.PanFingerRange                = { 2:true, 3:true }; // How many finger can be used to pan the camera
	this.PivotFingerRange              = { 1:true }; // How many fingers can be used for pivoting
	this.ZoomFingerRange               = { 2:true, 3:true }; // Number of finger that can be used for zooming
	this.ListenerPreventDefaults       = 
	{ 
		devicemotion:false, 
		touchstart:true, 
		touchmove:true, 
	...
	Object.defineProperty( this, 'PanSpeed', {
		...
		get:function(){
			...
			console.warn("DEPRECATED: Use engine.Speeds instead");
			return _engine.Speeds.pan.mouse;

		},
		set: function(val){

			console.warn("DEPRECATED: Use engine.Speeds instead");
			_engine.Speeds.pan.mouse = val;
			_engine.Speeds.pan.touch = val;

		}

	});

	Object.defineProperty( this, 'PivotSpeed', {

		get:function(){

			console.warn("DEPRECATED: Use engine.Speeds instead");
			return _engine.Speeds.pivot.mouse;

		},
		set: function(val){

			console.warn("DEPRECATED: Use engine.Speeds instead");
			_engine.Speeds.pivot.mouse = val;
			_engine.Speeds.pivot.touch = val;
	...
	Object.defineProperty( this, 'ZoomSpeed', {
		...
		get:function(){
			...
			console.warn("DEPRECATED: Use engine.Speeds instead");
			return _engine.Speeds.zoom.mouse;

		},
		set: function(val){

			console.warn("DEPRECATED: Use engine.Speeds instead");
			_engine.Speeds.zoom.mouse = val;
			_engine.Speeds.zoom.touch = val;
	...
	*/
	...
	this.NormaliseCameraLimits = function () {

		// Used to fix issues with cameras already exceeding their limits when the limits are applied.
		// This change the limits to wrap around the current camera position.
		// WARNING this won't always work as you want it to, we're assuming the limits are less than 2Pi difference.

		while ( this.rotLimits.xMin > this.RadialCam.target.x && this.rotLimits.xMin != null ){
			this.rotLimits.xMin -= 2*Math.PI;
		}
		while ( this.rotLimits.xMin < this.RadialCam.target.x - 2*Math.PI && this.rotLimits.xMin != null ){
			this.rotLimits.xMin += 2*Math.PI;
		}
		while ( this.rotLimits.yMin > this.RadialCam.target.y && this.rotLimits.yMin != null ){
			this.rotLimits.yMin -= 2*Math.PI;
		}
		while ( this.rotLimits.yMin < this.RadialCam.target.y - 2*Math.PI && this.rotLimits.yMin != null ){
			this.rotLimits.yMin += 2*Math.PI;
		}
		while ( this.rotLimits.zMin > this.RadialCam.target.z && this.rotLimits.zMin != null ){
			this.rotLimits.zMin -= 2*Math.PI;
		}
		while ( this.rotLimits.zMin < this.RadialCam.target.z - 2*Math.PI && this.rotLimits.zMin != null ){
			this.rotLimits.zMin += 2*Math.PI;
		}

		while ( this.rotLimits.xMax > this.RadialCam.target.x + 2*Math.PI && this.rotLimits.xMax != null ){
			this.rotLimits.xMax -= 2*Math.PI;
		}
		while ( this.rotLimits.xMax < this.RadialCam.target.x && this.rotLimits.xMax != null ){
			this.rotLimits.xMax += 2*Math.PI;
		}
		while ( this.rotLimits.yMax > this.RadialCam.target.y + 2*Math.PI && this.rotLimits.yMax != null ){
			this.rotLimits.yMax -= 2*Math.PI;
		}
		while ( this.rotLimits.yMax < this.RadialCam.target.y && this.rotLimits.yMax != null ){
			this.rotLimits.yMax += 2*Math.PI;
		}
		while ( this.rotLimits.zMax > this.RadialCam.target.z + 2*Math.PI && this.rotLimits.zMax != null ){
			this.rotLimits.zMax -= 2*Math.PI;
		}
		while ( this.rotLimits.zMax < this.RadialCam.target.z && this.rotLimits.zMax != null ){
			this.rotLimits.zMax += 2*Math.PI;
		}
	...
	this.LoadMaterialFromJSON = function ( json, onLoad, onProgress, onError ) {
		...
		PIKENG.getJSON(json, function(err, data) {
			...
			if ( data.scale ){
				...
				scaleX = parseFloat(data.scale);
				scaleY = parseFloat(data.scale);
			...
			if (data.alphaMap != false ){
				...
				material.setAlphaMap({ "scaleX":scaleX, "scaleY":scaleY ,"url":parentPath + data.alphaMap}, mapLoadingManager.succeeded.bind(mapLoadingManager), null, mapLoadingManager.failed);
			}

			if (data.normalMap != false ){
				mapLoadingManager.load();
				material.setNormalMap({ "scaleX":scaleX, "scaleY":scaleY ,"url":parentPath + data.normalMap}, mapLoadingManager.succeeded.bind(mapLoadingManager), null, mapLoadingManager.failed.bind(mapLoadingManager));
			}

			if (data.metalnessMap != false ){
				mapLoadingManager.load();
				material.setMetalnessMap({ "scaleX":scaleX, "scaleY":scaleY ,"url":parentPath + data.metalnessMap}, mapLoadingManager.succeeded.bind(mapLoadingManager), null, mapLoadingManager.failed.bind(mapLoadingManager));
			}

			if (data.roughnessMap != false ){
				mapLoadingManager.load();
				material.setRoughnessMap({ "scaleX":scaleX, "scaleY":scaleY ,"url":parentPath + data.roughnessMap}, mapLoadingManager.succeeded.bind(mapLoadingManager), null, mapLoadingManager.failed.bind(mapLoadingManager));
			}

			if (data.lightMapOverride != false ){
				mapLoadingManager.load();
				material.setLightMap({ "scaleX":scaleX, "scaleY":scaleY ,"url":parentPath + data.lightMapOverride}, mapLoadingManager.succeeded.bind(mapLoadingManager), null, mapLoadingManager.failed.bind(mapLoadingManager));
			}

			if (data.diffuse != false ){
				mapLoadingManager.load();
				material.setDiffuse({ "scaleX":scaleX, "scaleY":scaleY ,"url":parentPath + data.diffuse}, mapLoadingManager.succeeded.bind(mapLoadingManager), null, mapLoadingManager.failed.bind(mapLoadingManager));
			}

			if (data.metalness != null && data.metalness != undefined)
				material.metalness = data.metalness;

			if (data.envMapIntensity != null && data.envMapIntensity != undefined)
	...
	this.LoadModelsFromJSON = function ( json, name, group, onLoad, onProgress, onError ) {
		...
		PIKENG.getJSON(json, function(err, data) {
			...
			if (group !== false) { // If group is set to false, don't add the model to anything.

				parent = this.GetObjectNamed(group);

				if ( parent === undefined && typeof group == "string" ){
	...
	*/
	this.LoadLightingFromJson = this.LoadLightingFromJSON = function ( json, onLoad, onProgress, onError, duration ) {

		PIKENG.getJSON(json, function(err, lighting) {

			var i, len;

			this.Renderer.toneMapping = parseInt(lighting.toneMapping);
			...
			for (i=this.Lights.children.length-1; i>=0; i-- ){
			...
			for (i=0, len=lighting.lights.length; i<len; i++){
				...
				var lightNew = new THREE[lighting.lights[i].type]();

				lightNew.color.setHex("0x" + lighting.lights[i].color);
				if (lighting.lights[i].groundColor)
					lightNew.groundColor.setHex("0x" + lighting.lights[i].groundColor);
	...
	this.RemoveModelNamed = function ( modelName ){
		...
		var objects = _engine.TravsereSceneForObjectsNamed(modelName);
	...
	this.GetObjectNamed = function ( name, parent ){
		...
		if (parent)
			return parent.getObjectByName( parent );
	...
	this.Render = function ( once ) {
		...
		if (this.CameraAnimation) {
			...
			if ( this.CameraAnimation.endTime < time ){
		...
		var currentFrameTime = time / 1000;
		if (this.FrameTime != 0) {

			var frameDelta = currentFrameTime - this.FrameTime;
			this._UpdateVelocities ( frameDelta );
	...
	this._UpdateVelocities = function () {

		var dX, dY;
		var time = Date.now();
		var timeDifference = time - _InputPosition.timeStamp;

		if ( !_InputPosition.timeStamp || !_InputScale.timeStamp || _engine.TouchBlocker ){

			_InputPosition.timeStamp = time;
			_InputScale.timeStamp = time;
			return;
		...
		dx = (_NewInputPosition.x - _InputPosition.x)/(timeDifference);
		dy = (_NewInputPosition.y - _InputPosition.y)/(timeDifference);

		if (dx == 0 && dy == 0)
			return;

		if ( (_Panning || _Pivoting || _Zooming) && _engine.MoveCancelsCamAnim && _engine.CameraAnimation ){

			_engine.HaltCameraAnimation();

		}

		if ( _Panning && _engine.PanEnabled && !_engine.CameraAnimation ){

			_engine.PanVelocity.dy = dy/_Canvas.offsetHeight;
			_engine.PanVelocity.dx = dx/_Canvas.offsetWidth;

		}

		if ( _Pivoting && _engine.PivotEnabled && !_engine.CameraAnimation ){

			_engine.PivotVelocity.dy = dy/_Canvas.offsetHeight;
			_engine.PivotVelocity.dx = dx/_Canvas.offsetWidth;

		}

		if ( _Zooming && _engine.ZoomEnabled && !_engine.CameraAnimation ) {

			_engine.ZoomVelocity = -(_NewInputScale.scale - _InputScale.scale)/(timeDifference);

		}

		_InputPosition = _NewInputPosition;
		_InputPosition.timeStamp = time;
		_InputScale.scale = _NewInputScale.scale;
		_InputScale.timeStamp = time;
	...
	this._ApplyVelocities = function (){
		...
		if( this.PivotVelocity.dx != 0 || this.PivotVelocity.dy != 0 || this.PanVelocity.dx != 0 || this.PanVelocity.dy != 0 ){
			...
			var xrot = 2*(this.RadialCam.camera.fov/60)*this.PivotVelocity.dx*_engine.Speeds.pivot[_InputType];
			var yrot = 2*(this.RadialCam.camera.fov/60)*this.PivotVelocity.dy*_engine.Speeds.pivot[_InputType];
			...
			var vector = new THREE.Vector3(
				up.x*this.PanVelocity.dx*radius*_engine.Speeds.pan[_InputType],
				up.y*this.PanVelocity.dx*radius*_engine.Speeds.pan[_InputType],
				-up.z*this.PanVelocity.dx*radius*_engine.Speeds.pan[_InputType]);
			...
			var pos = this.RadialCam.pivotObject.getWorldPosition();
			var vector = new THREE.Vector3(
				up.x*this.PanVelocity.dy*radius*_engine.Speeds.pan[_InputType],
				up.y*this.PanVelocity.dy*radius*_engine.Speeds.pan[_InputType],
				up.z*this.PanVelocity.dy*radius*_engine.Speeds.pan[_InputType]
				);
			this.RadialCam.pivotObject.localToWorld(vector);
			...
			//Apply Zoom velocities

			if ( this.RadialCam.radius == 0 && _engine.ZoomVelocity ){

				this.RadialCam.camera.fov = this.RadialCam.camera.fov +_engine.Speeds.zoomFov[_InputType]*_engine.ZoomVelocity;

				if ( this.RadialCam.camera.fov > _engine.FovMax ){

					this.RadialCam.camera.fov = _engine.FovMax;

				}else if ( this.RadialCam.camera.fov < _engine.FovMin ){

					this.RadialCam.camera.fov = _engine.FovMin;

				}
 
			}else if ( _engine.ZoomVelocity ){
			
				this.RadialCam.radius = this.RadialCam.radius + _engine.Speeds.zoomRadius[_InputType]*_engine.ZoomVelocity;

				if ( this.RadialCam.radius > _engine.RadiusMax ){

					this.RadialCam.radius = _engine.RadiusMax;

				}else if ( this.RadialCam.radius < _engine.RadiusMin ){

					this.RadialCam.radius = _engine.RadiusMin;
			...
			if ( (this.PanVelocity.dx || this.PanVelocity.dy) && radius && this.PanSpeed )
				this.dispatchEvent( { type: 'CameraPanned', message: 'Camera Panned', data: null } );

			if ( _engine.ZoomVelocity )
				this.dispatchEvent( { type: 'CameraZoomed', message: 'Camera Zoomed.', data: null } );
	...
	this._ApplyDecelerations = function( frameDelta ){
		...
		*/
		...
		var zoomDeceleration = this.ZoomDeceleration;
		...
		*/
		if ( (this.PivotVelocity.dx < 0.000001  && this.PivotVelocity.dx > -0.000001) || this.Speeds.pivot.mouse == 0 || this.Speeds.pivot.touch == 0 ){
		...
		if ( this.PivotVelocity.dy < 0.000001  && this.PivotVelocity.dy > -0.000001 || this.Speeds.pivot.mouse == 0 || this.Speeds.pivot.touch == 0 ){
		...
		if ( (this.PanVelocity.dx < 0.000001  && this.PanVelocity.dx > -0.000001) || this.Speeds.pan.mouse == 0 || this.Speeds.pan.touch == 0 ){
		...
		if ( this.PanVelocity.dy < 0.000001  && this.PanVelocity.dy > -0.000001 || this.Speeds.pan.mouse == 0 || this.Speeds.pan.touch == 0 ){
		...
		if ( this.ZoomVelocity < 0.000001 && this.ZoomVelocity > -0.000001 ){

			this.ZoomVelocity = 0;

		}else if (this.ZoomVelocity != 0 ){

			this.ZoomVelocity = this.ZoomVelocity / ((zoomDeceleration)*frameDelta+1);
	...
	this.AnimateCamera = function ( start, end, duration, callback, type ){
		...
		if (!start.fov)
			start.fov = _engine.RadialCam.camera.fov;
		if (!end.fov)
			end.fov = _engine.RadialCam.camera.fov;
		...
		this.CameraAnimation = [
			this.makeAnimation( 'x', start.origin.x, end.origin.x, null, type ),
			this.makeAnimation( 'y', start.origin.y, end.origin.y, null, type ),
			this.makeAnimation( 'z', start.origin.z, end.origin.z, null, type ),
			this.makeAnimation( 'x', start.target.x, end.target.x, null, type ),
			this.makeAnimation( 'y', start.target.y, end.target.y, null, type ),
			this.makeAnimation( 'z', start.target.z, end.target.z, null, type ),
			this.makeAnimation( 'radius', start.radius, end.radius, null, type ),
			this.makeAnimation( 'fov', start.fov, end.fov, null, type )
		];
		...
		// Remove all velocities before animating
		this.PivotVelocity = { dx:0, dy:0 };
		this.PanVelocity = { dx:0, dy:0 };
		...
		this.CameraAnimation[6].Run(this.RadialCam, startTime, endTime, function(){

			this.engine.NormaliseCameraLimits();

		});
		this.CameraAnimation[7].Run(this.RadialCam.camera, startTime, endTime);
	...
	this.HaltCameraAnimation = function () {
		...
		this.dispatchEvent( { type: 'CameraAnimationHalted', message: 'Camera Animation Halted.', data: null } );
	...
	this.AddInputListeners = function (event){
		...
		this.Canvas.addEventListener( 'mousewheel', this.MouseWheelEvent, supportsPassive ? { passive: false } : false );
		this.Canvas.addEventListener( 'wheel', this.WheelEvent, supportsPassive ? { passive: false } : false );
		this.Canvas.addEventListener( "touchstart", this.TouchStartEvent, supportsPassive ? { passive: false } : false );
		this.Canvas.addEventListener( "touchend", this.TouchEndEvent );
		this.Canvas.addEventListener( "touchmove", this.TouchMoveEvent, supportsPassive ? { passive: false } : false );
	...
	this.MouseOverEvent = function (event){
		...
		if (_engine.ListenerPreventDefaults.mouseover)
	...
	this.MouseOutEvent = function (event){
		...
		if (_engine.ListenerPreventDefaults.mouseout)
			event.preventDefault();
	...
	this.MouseUpEvent = function ( event ){
		...
		if (_engine.ListenerPreventDefaults.mouseup){
	...
	this.MouseMoveEvent = function (event){
		...
		if (_engine.ListenerPreventDefaults.mousemove)
			event.preventDefault();
	...
	this.MouseDownEvent = function (event){
		...
		if (_engine.ListenerPreventDefaults.mousedown)
			event.preventDefault();
	...
	this.ContextMenuEvent = function (event){
		...
		if (_engine.ListenerPreventDefaults.contextmenu)
			event.preventDefault();
	...
	this.TouchMoveEvent = function(event){
		...
		_engine.dispatchEvent( { type: 'TouchMove', message: 'Touch moved', data: null, originalEvent: event } );

		if (_engine.ListenerPreventDefaults.touchmove)
			event.preventDefault();

		if (_engine.TouchBlocker)
			_engine.TouchBlocker = false;

		// Calculate the new average input position of the touches.
		_NewInputPosition = _engine.AverageTouchPositions(event);
		_NewInputScale.scale = _engine.TouchScale(event);

		// if( !_TouchScale )
		// 	_TouchScale = event.scale;

		// Uer moved finger, it's not a tap anymore.
		if (_ListenForTap && _NewInputPosition.x != _InputPosition.x && _NewInputPosition.y != _InputPosition.y )
			_ListenForTap = false;
	...
	this.TouchEndEvent = function(event) {
		...
		if ( _engine.ListenerPreventDefaults.touchend );
			event.preventDefault();

		if ( _engine.PanFingerRange[event.targetTouches.length] ){
		...
		if ( _engine.PivotFingerRange[event.targetTouches.length] ){
		...
		if ( _engine.ZoomFingerRange[event.targetTouches.length] ) {
		...
		if (event.targetTouches.length > 0){
			...
			_InputScale.scale = _engine.TouchScale(event);
			var newInputPosition = _engine.AverageTouchPositions(event);
			_InputPosition.x = newInputPosition.x;
			_InputPosition.y = newInputPosition.y;

		}

		_engine.TouchBlocker = true;
		...
		_engine.dispatchEvent( { type: 'TouchUp', data: event, message: 'Touch Up.' } );

	}

	this.WheelEvent = function(event) {

		if (_engine.ListenerPreventDefaults.wheel)
			event.preventDefault();
	...
	this.TouchStartEvent = function(event) {
		...
		if (_engine.ListenerPreventDefaults.TouchStart)
			event.preventDefault();
		...
		_InputPosition = _engine.AverageTouchPositions(event);
		_InputScale.scale = _engine.TouchScale(event);
	...
	this.TouchScale = function (event) {
		...
		for ( i=0; i<event.targetTouches.length; i++){
			...
			for ( j=0; j<event.targetTouches.length; j++ ){
	...
	this.AverageTouchPositions = function (event) {

		var newInputPosition = new THREE.Vector2();
		var len =  event.targetTouches.length;

		for ( var i=0; i<len; i++ ){

			newInputPosition.x += event.targetTouches[i].clientX;
			newInputPosition.y += event.targetTouches[i].clientY;

		}

		if ( event.targetTouches.length > 1 ){
			newInputPosition.x /= event.targetTouches.length;
			newInputPosition.y /= event.targetTouches.length;
		}
	...
	this.MouseWheelEvent = function(event) {
		...
		if (_engine.ListenerPreventDefaults.mousewheel)
			event.preventDefault();
	...
	this.Input2Down = function (event){
		...
		// Mouse events
		_InputPosition.x = event.pageX;
		_InputPosition.y = event.pageY;
	...
	this.InputMove = function (event){
		...
		newInputPosition.x = event.pageX;
		newInputPosition.y = event.pageY;
	...
	this.HandleMotionEvent = function (event){
		...
		if (_engine.ListenerPreventDefaults.devicemotion)
			event.preventDefault();
	...
	this.PanCamera = function (event){
		...
		this.dispatchEvent( { type: 'CameraPanning', message: 'Camera About to Pan.', data: location, originalEvent: event } );

		// check to see if we're currently disabled. Run after event dispatcher so listeners can override
		if (!this.PanEnabled) {
			return;
		}
		...
		if (event.pageX){
			dX = (event.pageX - _InputPosition.x)/(timeDifference);
			dY = (event.pageY - _InputPosition.y)/(timeDifference);
		}else{
			dX = (event.changedTouches[0].clientX - _InputPosition.x)/(timeDifference);
			dY = (event.changedTouches[0].clientY - _InputPosition.y)/(timeDifference);
		}

		if (this.CameraAnimation && this.MoveCancelsCamAnim )
			this.HaltCameraAnimation();
		else if (!this.CameraAnimation){
	...
	this.PivotCamera = function (event){
		...
		this.dispatchEvent( { type: 'CameraPivoting', message: 'Camera About to Pivot.', data: location, originalEvent: event } );
	...
	this.InputZoom = function (event){
		...
		this.dispatchEvent( { type: 'CameraZooming', message: 'Camera Zooming.', data: location, originalEvent: event } );
		...
		if (this.RadialCam.radius < this.RadiusMin && this.RadialCam.radius != 0 && this.RadiusMin != null )
			this.RadialCam.radius = this.RadiusMin;

		if (this.RadialCam.radius > this.RadiusMax && this.RadiusMax != null )
			this.RadialCam.radius = this.RadiusMax;
...
PIKENG.SetMap = function ( type, attributes, onLoad, onProgress, onError ) {
	...
	function(texture){
		...
		if ( (type == 'map' || type == 'metalnessMap' || type == 'specularMap' || type == 'roughnessMap' || type == 'normalMap' || type == 'alphaMap') ){

			texture.repeat.width = this.scaleX;
			texture.repeat.height = this.scaleY;
...
THREE.MeshStandardMaterial.prototype.setRoughnessMap = function ( attributes, onLoad, onProgress, onError ){
...
var setAlphaMap = function ( attributes, onLoad, onProgress, onError ){

	PIKENG.SetMap.call( this, 'alphaMap', attributes, onLoad, onProgress, onError );
...
THREE.MeshPhongMaterial.prototype.setSpecularMap = function ( attributes, onLoad, onProgress, onError ){

	PIKENG.SetMap.call( this, 'specularMap', attributes, onLoad, onProgress, onError );

}

THREE.MeshStandardMaterial.prototype.setMetalnessMap = function ( attributes, onLoad, onProgress, onError ){

	PIKENG.SetMap.call( this, 'metalnessMap', attributes, onLoad, onProgress, onError );
...
var setLightMap = function ( attributes, onLoad, onProgress, onError ){

	PIKENG.SetMap.call( this, 'lightMap', attributes, onLoad, onProgress, onError );
...
var setNormalMap = function ( attributes, onLoad, onProgress, onError ){

	PIKENG.SetMap.call( this, 'normalMap', attributes, onLoad, onProgress, onError );
...
THREE.MeshPhongMaterial.prototype.setAOMap = function ( attributes, onLoad, onProgress, onError ){

	PIKENG.SetMap.call( this, 'aoMap', attributes, onLoad, onProgress, onError );

}

var setBumpMap = function ( attributes, onLoad, onProgress, onError ){

	PIKENG.SetMap.call( this, 'bumpMap', attributes, onLoad, onProgress, onError );
...
THREE.Object3D.prototype.applyMaterialToId = function (material, id) {
	...
	for (i=0; i<len; i++){
		...
		if (mesh.materialId == id){
			...
			if ( mesh.lightMapIntensity && !material.lightMapOverrideIntensity ) // Apply light map intensity
				material.lightMapIntensity = mesh.lightMapIntensity;
			mesh.material = material;
...
PIKENG.RadialPosition = function (origin, target, radius, fov) {
	...
	this.target = target || new THREE.Euler(1.57,0,0);
...
PIKENG.Animation.prototype.Run = function ( target, start, end, completed ) {
